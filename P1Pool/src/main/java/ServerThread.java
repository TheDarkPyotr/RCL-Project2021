...
class ServerThread implements Runnable {
    ...
    public void run() {
        ...
        System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+  "] New connection established with " + s.getInetAddress());
        ...
        while (request.compareTo("EXIT") != 0) {
            ...
            try {
                ...
                System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+  "] ERROR: request loop error!");
                e.printStackTrace();
            ...
            switch(choice) {
                case "LOGIN-REQUEST":
                    ...
                    System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+ "]  Received " + choice + " request from " + s.getInetAddress() + " with data: " + tokens[1]);
                    response =  "";
                    try {
                    ...
                    os.println(response);
                    os.flush();
                    System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+ "] Response to login request from" + s.getInetAddress() + " with: " + response +  "\n");
                ...
                case "LOGOUT-REQUEST":
                    ...
                    if(tokens.length == 1){
                        System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+ "] Close TCP connection with non authenticated user");
                        response = "OK";
                    } else{
                         // Close TCP connection and change user status, for authenticated user
                        if(data.logout(tokens[1]) == 1) response = "ERROR";
                        else response = "OK";
         ...
         */
                    ...
                    Integer operationStatus = data.moveCard(tokens[1],tokens[2],tokens[3]);
                ...
                case "CREATE-PROJECT-REQUEST":
                    ...
                    response = "OK";
                    operationStatus = this.data.newProject(tokens[1],authenticatedUsername);
                    if (operationStatus == 1) response = "PROJECT-TITLE-ERROR";
                    else if(operationStatus == 2) response = "MULTICAST-LIMIT";
        ...
        try {
            ...
            System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+ "]] Shutting down connection with " + s.getInetAddress());
        } catch (IOException e) {
            System.out.println("[SERVER] Error during shutdown connection with " + s.getInetAddress());
            e.printStackTrace();
        }

        System.out.println("[SERVER THREAD " + Thread.currentThread().getId()+ "]  Execution terminated, bye!");
        Thread.currentThread().interrupt();
    }

    public void chatStateUpdater(String projectName, String cardName, String destinationList) throws IOException {

        //Send state card update message on multicast group for project projectName
